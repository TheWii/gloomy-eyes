...
execute store success score #summon_chance twvp.temp if score #result twvp.math <= @s twge.chance


# Proceed
scoreboard players set #success_summons twvp.temp 0
scoreboard players set #loop twvp.temp 1
execute if score #summon_chance twvp.temp matches 1 at @s anchored eyes positioned ^ ^ ^ run function thewii:gloomy_eyes/player/check/raycast


# Reset chance if succeeded
execute if score #success_summons twvp.temp matches 1.. run scoreboard players operation @s twge.chance = #eyes.default_summon_chance twge.data
...
#execute if score #summon_chance twvp.temp matches 0 run tellraw @s ["Chance failed, increased chance to ",{"score":{"name":"#chance_int","objective":"twvp.temp"}},",",{"score":{"name":"#chance_dec","objective":"twvp.temp"}},"%."]
#execute if score #summon_chance twvp.temp matches 1 if score #success_summons twvp.temp matches 0 run tellraw @s ["Failed to summon, increased chance to ",{"score":{"name":"#chance_int","objective":"twvp.temp"}},",",{"score":{"name":"#chance_dec","objective":"twvp.temp"}},"%."]
